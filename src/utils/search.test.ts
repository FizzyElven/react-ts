...
describe("searchInArray", ()=>{
    ...
    it("if object property includes value, then it should be in returned array", ()=>{
        expect(searchInArray<{name: string}, "name">([{name: "Alice"}, {name: "Alex"}], "Al", ["name"])).toEqual([{name: "Alice"}, {name: "Alex"}])
    })
    it("if object property not includes value, then it should filtered", ()=>{
        expect(searchInArray<{name: string}, "name">([{name: "Alice"}, {name: "Alex"}], "x", ["name"])).toEqual([{name: "Alex"}])
    })
})